...
Claude Code auto-injects `CLAUDE.md` (and any `.claude/memory/MEMORY.md` index) at the start
of **every** session. Those always-loaded files are the "information poisoning" vector. The
official guidance is explicit: _task-specific/handoff instructions belong in a **skill** that
loads only when invoked, not in always-on memory_ - which is exactly what this is.
**claude-transfer never writes to any auto-loaded path.**
...
5. **Review in plan mode**, then on approval (create the folder if missing):
   - Write `.claude/workspace/reports/transfers/claude/{YYYY-MM-DD}-{topic}.md`, where
     `{topic}` names **what the session did** (e.g. `add-review-pagination`), **not** this
     skill's name.
...
3. **Re-ground before acting** (do NOT start new work first): read the brief's pointers + the
   recent `git log`, then run a **basic functional check** (e.g. `just lint`, or `just start`
   and load http://127.0.0.1:8112/login.php) to catch drift or undocumented breakage. Building a
   new feature on a silently-broken state makes it worse. Report any drift you find.
...
- Files: `login.php:255-330`, `db_config.php`
- Recent commits: `<hash> <subject>` (run `git log --oneline -5`)
- Branch: {branch} PR: #{id}
- Commands: `just start`, `just lint`, `just stop`
...
- {fact} - source: `path:line` / `Tests: N passed` / command output
...
- Dev server on :8112 was {up/down} at close - re-check (`just start` / `just stop`)
- Uncommitted changes: {yes/no} - `git status`
- MySQL on localhost:3307: {up/down} as of {time} - DB pages 500 without it
...
- Ported from book-review for my-penawar (plain PHP). Commands + freshness checks adapted
  to this stack (`just start` on :8112, `just lint`, MySQL-on-3307 availability).
