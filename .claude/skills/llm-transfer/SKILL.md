...
## 1. Role
You are {persona - e.g. a senior PHP 8 engineer who maintains legacy procedural code / a
MySQL query reviewer / a web-security specialist}. {Any relevant seniority, domain, or mindset.}
...
situation: what the project is (a plain-PHP clinic-booking site for Poliklinik Penawar:
login/register, appointment booking, staff appointment admin, receipts), the domain, and WHY
this task matters. Self-contained - assume the reader has never seen this codebase.}
...
## 6. Constraints & Rules
- Tech stack: {plain PHP 8.4 (no framework) / mysqli / MySQL-MariaDB on localhost:3307 /
  Bootstrap 4 + jQuery from CDNs, versions}
...
```text
Saved: .claude/workspace/reports/transfers/{tool}/2026-08-01-{topic}.md
Feed it:  (gpt/gemini) paste into the web UI   |   (ollama) cat <file> | ollama run <model>
```

## Worked example (compact)

Target: _"Hand off a broken login query to an external LLM for a fix."_

````text
# MASTER PROMPT - Fix staff login always failing after PHP 8.4 upgrade

## 1. Role
You are a senior PHP engineer who writes precise, minimal fixes for legacy procedural code.

## 2. Mission
Make the staff branch of the login handler work on PHP 8.4 + mysqli, and explain the root
cause in one paragraph.

## 3. Background & Context
Plain-PHP clinic site (no framework). login.php renders a login/register form and, at the
bottom of the same file, connects to MySQL via mysqli and checks the posted id/password
against the `employee` table. Since PHP 8.1, mysqli throws exceptions on failure instead of
returning false. No repo access - everything you need is below.

## 4. Current State
- Done: patient login works when MySQL is running.
- Tried: re-importing mypenawar.sql - no effect.
- Failing: staff login dies with an uncaught mysqli_sql_exception.

## 5. Relevant Artifacts
`login.php` (staff branch, excerpt)
```
$sql= "SELECT *
       FROM employee
...
## 6. Constraints & Rules
- PHP 8.4, procedural mysqli. Keep the page-per-file structure; no framework.
- New code must not interpolate POST values into SQL (use a prepared statement).

## 7. Your Task
Return the corrected staff-login block and a one-paragraph root-cause explanation.

## 8. Output Format & Success Criteria
- Output: the fixed PHP block only + the explanation.
- Done: valid staff credentials redirect to `employee profile.php`; invalid ones re-show
  login.php; no uncaught exception.

## 9. Open Questions & Assumptions
- Assume the `employee` table matches mypenawar.sql - note if that matters.

## Reminder
Return the fixed staff-login block + a one-paragraph root cause. Keep the file structure.
````
...
- Ported from book-review for my-penawar (plain PHP). Personas, stack constraints, and the
  worked example re-grounded on this repo (procedural mysqli login flow, MySQL on :3307).
