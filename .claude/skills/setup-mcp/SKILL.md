...
- **Job A — set up an already-registered server** for a developer (the roster in the CLAUDE.md
  `## MCP Servers` section): resolve the name → its `registry.json` record → wire config + enablement → verify.
...
1. **Config** — real values live in the **git-ignored `.mcp.json`**; the committed `.mcp.json.stub`
   ...
   never overwrite or duplicate. To set up on a fresh machine: `setup.ps1` copies
   `.mcp.json.stub` → `.mcp.json` for you; fill real secrets in `.mcp.json` only.
2. **Enablement** by the record's `enableTier`:
   ...
   - `per-dev` (secret: github) → add the name to `enabledMcpjsonServers` in the
...
3. **Secrets never in git.** A PAT/token goes only in `.mcp.json` (git-ignored). This repo has no
   secret-scanning hooks, so double-check `git status` before every commit — the `.gitignore`
   rules for `.mcp.json` / `settings.local.json` are the only guard.
...
2. **Read the record** — note `installType`, `secret`, `enableTier`, `endpointOrPackage`, and the `setup`
   note. Follow the `setup` note verbatim (e.g. github PAT scopes, playwright browser install).
...
5. **Docs** — add one row to the CLAUDE.md `## MCP Servers` section (purpose, secret, enable tier). No new
...
- **Never** put a real token/PAT/password in any committed file (SKILL.md, `registry.json`, the stub,
  `settings.json`). Placeholders / KEY names only.
