...
enabled for this dev/machine is a **SKIP** (not a FAIL). A server enabled with
no check prompt is a gap → note it (author `checks/<server>.txt`).
...
| **SKIP** | not enabled for this dev, or a precondition is absent (app server down, browser binary missing, PAT not configured)           |

**Bake in the known non-failures** — do not misread these as FAIL:

- **playwright (127.0.0.1:8112)** — needs `just start` running. With the app stopped, a
  connection-refused is a SKIP, not a FAIL (playwright can retest against about:blank).
...
context7    turnkey     yes        PASS     resolved /php/php-src
playwright  turnkey     yes        PASS     snapshot returned
github      per-dev     -          SKIP     not opted in / no PAT
```
