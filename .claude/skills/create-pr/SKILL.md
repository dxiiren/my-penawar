...
- If there are multiple commits, synthesize one Conventional title that summarizes
  the change, e.g. `feat(patient): validate booking date before insert` or
  `fix(auth): escape login alert output`. Pick the type/scope from the
...
- <bullet per meaningful change — page/query/style touched>
- <...>

## Testing

- <how it was verified: `just lint` result, manual check on http://127.0.0.1:8112, etc.>
```
...
title used. This repo has no CI, so there are no checks to wait for — review and
merge are manual.
...
| `/pre-pr-review` | Optional self-review (plain-PHP/mysqli checklist) before opening the PR       |
| `/lint-check`    | Run the php -l quality suite before the PR                                    |
