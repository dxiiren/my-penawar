...
- **One theme per round.** React to each answer before moving on. If an answer is vague ("make it
  fast", "handle the pages"), drill in until it's checkable ("list page renders < 200ms with the
  cache warm", "these 3 named routes").
...
2. Is the work a **known finite list**, or must it be **discovered first**? If discovered - by what
   (grep / glob over the root `*.php` pages / a curl sweep of every page)? Roughly how many items?
3. What proves **one** item is done? (Push for an evidence artifact - a pasted `just lint` line,
   an HTTP 200 from `curl.exe`, a report path - not just a feeling.)
...
5. May it `git commit` / `push` / stage? May it open a GitHub PR/issue, or touch any external
   service? Any destructive ops (e.g. re-importing `mypenawar.sql` wipes the local db) or
   off-limits paths (`Mail/phpmailer/` is vendored - never edit it)?
...
- What's the before->after transformation, stated concretely enough to apply uniformly (e.g.
  interpolated SQL -> prepared statements, or inline connects -> `require 'db_config.php'`)?
- Isolation: do items touch shared files (-> serialize) or disjoint ones (-> can parallelize)?
- What's the per-item verification (`just lint` / a curl 200 check / grep) that proves the
  change is safe?
...
- This repo has **no deployment target** - it runs locally only. A "deployment" goal here means
  local serving (`just start` on :8112) or authoring future deploy docs. If the developer wants a
  real deploy, first interrogate WHERE (host, method) - that decision is itself part of the goal.
- What's the health-verify that proves success (an HTTP 200 from `curl.exe` on :8112, a page
  rendering with its form visible)?
