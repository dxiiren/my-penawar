...
4. **Guardrails & locked decisions** - may it `git commit` / `push` / stage? May it open a PR /
   comment on GitHub, or touch any external service? Any destructive ops (e.g. re-importing
   `mypenawar.sql` wipes the local db)? What decisions are **locked** (do-not-relitigate)? Any
   **environment bootstrap** (the dev server via `just start`, a sanity gate like `just lint`)?
5. **Real-blocker definition** - what counts as a _genuine_ blocker (a code fact: a 404 route, a
   missing class import, a hard infra failure it can't fix) versus a banned excuse ("it's late",
...
- Ported from book-review for my-penawar (plain PHP). Examples adapted to this stack
  (`just start` on :8112, `just lint` as the sanity gate, `mypenawar.sql` import as the
  destructive op); references re-grounded on this repo's pages and MySQL dependency.
