...
set so generic backticks like `` `composer` `` are not treated as orphans). When you
...
- **CRED_EXPOSURE** — replace the hardcoded value: in a script read it from
  `os.environ` / `env()`; in a doc reference the KEY name (e.g. `` `<DB_PASSWORD>` ``)
  or an env var — never the literal. The real value lives only in git-ignored config
  (`.env`, `.mcp.json`, `.claude/settings.local.json`). If a value is a genuinely
...
- Ported from book-review for my-penawar (plain PHP). `audit.py` copied verbatim
  (it is stack-agnostic); auto-fix guidance examples kept — the git-ignored config
  set (`.mcp.json`, `settings.local.json`) is identical here.
