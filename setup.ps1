...
# ErrorActionPreference is 'Stop'. PHP and a few other tools write non-fatal
# warnings to stderr -- disable this behavior so our [FAIL] messages are driven
# by exit codes, not by whether stderr was touched.
...
Write-Host ""
Write-Host "MyPenawar Bootstrap Setup" -ForegroundColor Cyan
...
if (-not $hasWinget) {
    Write-Host "[WARN] winget not found. Auto-install for Git will be skipped." -ForegroundColor Yellow
...
# ---------- 2. Claude Code CLI ----------
# This plain-PHP repo has no package.json, so Node.js is not a project toolchain here --
# npm is only the delivery vehicle for the Claude Code CLI. If npm is absent the step
# warns instead of failing: the app itself never needs Node.
Refresh-Path
if (Test-Command "claude") {
    ...
    Write-Host "[WARN] npm not found -- skipping Claude Code install." -ForegroundColor Yellow
    Write-Host "       Install Node.js LTS (winget install OpenJS.NodeJS.LTS), reopen PowerShell, re-run this script." -ForegroundColor DarkGray
}

# ---------- 3. uv (Python package manager / tool runner) ----------
Refresh-Path
if (Test-Command "uv") {
...
# ---------- 4. Python (used by .claude tooling, e.g. the statusline + skill scripts) ----------
...
# ---------- 5. Microsoft Visual C++ Redistributable (required by PHP) ----------
# PHP 8.4 is linked against a recent MSVC runtime. If SYSTEM32\VCRUNTIME140.dll
...
$phpDir = Join-Path $env:LOCALAPPDATA "Programs\php-8.4"
$phpExe = Join-Path $phpDir "php.exe"

Refresh-Path
if (Test-Path $phpExe) {
    $phpVer = & $phpExe --version 2>&1 | Select-Object -First 1
    Write-Host "[OK] PHP 8.4 already installed: $phpVer" -ForegroundColor Green
} else {
    $phpZipUrl = "https://windows.php.net/downloads/releases/latest/php-8.4-Win32-vs17-x64-latest.zip"
    $zipPath = Join-Path $env:TEMP "php-8.4-latest.zip"
    Write-Host "[INSTALL] Downloading PHP 8.4 (latest patch) from php.net..." -ForegroundColor Yellow
    try {
        ...
        if (Test-Path $phpExe) {
            $phpVer = & $phpExe --version 2>&1 | Select-Object -First 1
            Write-Host "[OK] PHP 8.4 installed: $phpVer" -ForegroundColor Green
        } else {
...
# The fresh PHP zip has no php.ini -- only ini-development / ini-production
# templates -- so we copy the development template and append the extensions
# this app needs (mysqli drives every DB-backed page via db_config.php).
if (Test-Path $phpExe) {
...
; --- my-penawar setup.ps1 overrides ---
...
"@
        ...
        Write-Host "[INFO] Wrote php.ini with the extensions this app needs (incl. mysqli)" -ForegroundColor Cyan
    }
    # php.ini may pre-exist from another project's setup on the same shared PHP dir --
    # make sure mysqli is enabled either way (idempotent: appends only when absent).
    if ((Test-Path $phpIni) -and (-not (Select-String -Path $phpIni -Pattern '^extension=mysqli' -Quiet))) {
        Add-Content -Path $phpIni -Value "`n; my-penawar: mysqli required by db_config.php`nextension=mysqli"
        Write-Host "[INFO] Enabled the mysqli extension in the existing php.ini" -ForegroundColor Cyan
    }
    Add-UserPath $phpDir
...
$missing = @()
foreach ($tool in @('git','claude','uv','just','gh')) {
